...
export class TruffleProvider implements IVotingProvider {
    ...
    async claimRewards(claims: RewardClaimWithProof[]): Promise<any> {
        ...
        for (const claim of claims) {
            const receipt = await this.contracts.votingRewardManager.claimReward(
                hexlifyBN(claim),
    ...
    async registerAsVoter(rewardEpochId: number, weight: number): Promise<any> {
        return await this.contracts.voterRegistry.registerAsAVoter(rewardEpochId, weight, {
            from: this.account.address,
        });
    }
    ...
    static async create(
        ...
        options: TruffleProviderOptions
    ): Promise<TruffleProvider> {
